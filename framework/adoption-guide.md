...
**Add platform adapters** when you work across products. Shared contract bodies should work alone and declare deferral when full files are loaded, within the host’s hierarchy. Product addenda and project templates declare the base context they require; maintenance references explain delivery without becoming runtime contracts. Preserve narrower preview and confirmation safeguards. A defined change request supplies scoped approval; it does not authorise unrelated actions or waive those safeguards. Maintain shared derivations once, generate detached copies, and record which full-contract material was deliberately omitted. Verify base-plus-addendum loading rather than assuming a host supplies both. Watch character limits: a condensed prompt near its platform's cap means every future amendment is a displacement decision.

**Add model guidance** only for an observed need or a relevant documented difference. Record the model, host, source date and evidence status in implementation. Keep shared policy stable across models, derive execution instructions by task, and check whether an override replaces provider instructions before using it. Native model settings may not pass through a gateway.

**Add a capability implementation** when a recurring task or credible example suggests useful value. Prefer the host provider's native or own published route; consistent outcomes across tools do not require cross-importing provider packages. Use upstream sources to identify offerings, check authorship separately from catalogue membership and inspect the selected workflow's dependencies and effects. Maintain one concise selection catalogue, with platform guides for configuration and dated evidence for observed results. Use actual enabled/disabled states and assess the whole bundle when the host enables all included skills together. Do not assume a disabled package remains available for task selection. Prefer managed provider distribution and record unavailable routes. Verify useful live work and compare against the native route when value or interference is uncertain; an exhaustive pilot is not a prerequisite for every selection. Independent third-party skills may be shared where useful and supported. A new marketplace or folder hierarchy is unnecessary until distribution needs justify it.

**Reassess additions when models or hosts change.** Test the current configuration against one with a suspect procedural workaround removed. Preserve personal policy and ownership; record actual quality, repairs, latency and usage where available. A smaller instruction set is a useful outcome when it retains the required behaviour. Do not infer savings from file size or catalogue count, and do not automatically replace a capable judgement model with a cheaper one.
...
**Keep memory out.** The knowledge store — whatever tool holds your notes and developing positions — is state, not system. The kit governs deliberate action at the boundary: what is worth keeping, what may be written back, and how anything written is labelled with its origin (yours, the model's, a source's). The store's internal structure belongs to the store. The operating contract should require explicit scoped approval for tool-mediated changes to persistent artefacts. A direct request for a specified change supplies that approval within its scope. A memory-facing role charter adds decision policy and workflow within this boundary; adding the role does not relax the requirement. Ambient memory and history managed automatically by a product belong to that product's settings and retention controls; a prompt should not claim authority over them.
...
- **Cross-model consistency.** Loaded into a different model, do the binding rules still hold? Variation in depth and style is expected; variation in adherence calls for checking instruction composition, conflicting host or skill rules, model behaviour and whether condensation omitted a protection. A clearer hierarchy alone does not guarantee parity.
