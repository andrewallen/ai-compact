...
**Add platform adapters** when you work across products. Each is a self-contained derivation of the operating contract that declares deferral when full files are loaded, within the host’s hierarchy. Preserve narrower preview and confirmation safeguards. A defined change request supplies scoped approval; it does not authorise unrelated actions or waive those safeguards. Maintain shared derivations once, generate detached copies, and record which full-contract material was deliberately omitted. Document base-plus-addendum loading rather than assuming a host supplies both. Watch character limits: a condensed prompt near its platform's cap means every future amendment is a displacement decision.
...
**Add a capability implementation** when a recurring task exposes a specific gap. Define acceptance first, inspect the native baseline, then compare one candidate with the same model/host and task conditions. Review the complete package, including scripts, hooks and service access, and verify publisher/component attribution separately from catalogue membership. Record exact revisions, selection, observed state and rollback limits. Keep the register at maintenance time and use client adapters for delivery; a new marketplace or folder hierarchy is unnecessary until distribution needs justify it.
