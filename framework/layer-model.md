...
Maintain one concise catalogue of selected capabilities and their upstream sources. Prefer the host provider's useful native or own published implementation; consistent outcomes need not require importing one provider's packages into another's tool. Check authorship separately from marketplace membership. Independently authored third-party skills can be shared where useful and supported. Platform guides own delivery and supported controls; dated evidence owns observed deployment and results. Keep these references outside ordinary runtime context. This requires no additional authority layer.

Scale adoption to the decision: establish fit and source, configure the relevant capability, verify useful live work, then refine. Use a matched native-versus-addition comparison when value, cost or interference is uncertain, rather than as a universal prerequisite. Model configuration using actual host controls. Where enablement applies to a whole plugin, assess every included skill and distinguish enabled availability from invocation. A disabled package may be unavailable for task selection; do not invent an intermediate state or impose per-task toggling. Prefer managed provider distribution and record unavailable delivery routes. Preserve ownership and behavioural boundaries, measure accepted-task quality and correction effort, and distinguish exposure from loading or successful use. More installed packages or fewer source bytes establish neither better capability nor token savings.
