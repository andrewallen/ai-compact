...
Maintain a small register for implementations actually under consideration. Separate publisher/component provenance, pinned source identity, selection and observed runtime state. Cache presence, installation, enablement, exposure, loading and successful execution need their own evidence. Keep records and evaluations outside ordinary runtime context; link each fact to one canonical owner. This requires no additional authority layer.

Compare one addition against the applicable native baseline on matched tasks before adopting it. Preserve ownership and behavioural boundaries; allow complementary task stages while avoiding competing primary workflows. Measure quality and total completion effort, including corrections, and retire obsolete procedures through explicit, scoped changes after model or host improvements. More packages do not establish better capability.
