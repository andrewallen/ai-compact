# Structured Critique

**What it does.** Produces a direct, grounded evaluation of a document, proposal, strategy, or argument — distinguishing structural issues from surface concerns and grounding every criticism in evidence from the document itself.

**When to use it.** Before submitting a document, when reviewing someone else's work, when stress-testing a strategy or proposal, or when you need the refinement mode to operate against explicit criteria rather than general improvement.

## Prompt

> Provide a structured critique of this [DOCUMENT TYPE].
>
> 1. State its core argument or purpose in one sentence
> 2. Identify its three strongest elements
> 3. Identify its three most significant weaknesses or gaps
> 4. For each weakness, suggest a specific improvement
> 5. Assess: does this achieve its stated purpose? What would make it substantially better?
>
> Be direct and specific. Ground critique in evidence from the document, not general principles. Distinguish between structural issues and surface-level concerns.

## Notes

The constraint to ground critique in the document prevents generic feedback. The distinction between structural and surface issues prevents the model from fixating on wording when the architecture is the problem. The three/three balance prevents the model from producing either a demolition or an endorsement.

Version: 2026.04.02 @ 1800